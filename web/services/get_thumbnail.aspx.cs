...
//public partial class services_get_thumbnail : System.Web.UI.Page
//{
//    // see http://weblogs.asp.net/bleroy/the-fastest-way-to-resize-images-from-asp-net-and-it-s-more-supported-ish
//    // see http://weblogs.asp.net/bleroy/what-interpolationmode-and-compositingquality-to-use-when-generating-thumbnails-via-system-drawing


//    private readonly Regex _nameValidationExpression = new Regex(@"[^\w/]+");
//    private const string PhotoPath = "../data/images/";
//    protected const string PhotoCachePath = "../data/cache/";
...
//        string photoName = context.Request.QueryString["p"] ?? String.Empty;
//        if (_nameValidationExpression.IsMatch(photoName))
//        {
//            throw new HttpException(404, "Invalid photo name.");
//        }
//        bool cache = !String.Equals(context.Request.QueryString["nocache"], "true", StringComparison.OrdinalIgnoreCase);
//        // Check out the cache
//        string cachePath = context.Server.MapPath(PhotoCachePath);
//        string photoCachePath = Path.Combine(cachePath, photoName + ".jpg");
...
//        string photoPath = context.Server.MapPath(PhotoPath + photoName + ".jpg");
//        try
//        {
//            // Read the source image
//            byte[] photo = File.ReadAllBytes(photoPath);
//            IWICComponentFactory factory = (IWICComponentFactory)new WICImagingFactory();
//            IWICStream inputStream = factory.CreateStream();
//            inputStream.InitializeFromMemory(photo, (uint)photo.Length);
//            IWICBitmapDecoder decoder = factory.CreateDecoderFromStream(inputStream, null, WICDecodeOptions.WICDecodeMetadataCacheOnLoad);
...
//            MemoryIStream outputStream = new MemoryIStream();
//            // Prepare JPG encoder
//            IWICBitmapEncoder encoder = factory.CreateEncoder(Consts.GUID_ContainerFormatJpeg, null);
//            encoder.Initialize(outputStream, WICBitmapEncoderCacheOption.WICBitmapEncoderNoCache);
//            // Prepare output frame
//            IWICBitmapFrameEncode outputFrame;
//            IPropertyBag2[] arg = new IPropertyBag2[1];
//            encoder.CreateNewFrame(out outputFrame, arg);
//            IPropertyBag2 propBag = arg[0];
//            PROPBAG2[] propertyBagOption = new PROPBAG2[1];
...
//            WICRect wicRect = new WICRect();
//            wicRect.X = 0;
//            wicRect.Y = 0;
//            wicRect.Width = (int)thumbWidth;
//            wicRect.Height = (int)thumbHeight;

//            outputFrame.WriteSource(scaler, wicRect);
//            outputFrame.Commit();
//            encoder.Commit();
