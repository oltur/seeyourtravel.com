...
function showPhotos(track,p)
{
    ...
    var urlp = "http://www.panoramio.com/map/get_panoramas.php?set="+set+"&from=0&to=" + track.numOfPhotos.toString() + "&miny="
        + (p.lat - track.photoLocationTolerancy).toString()
        + "&minx=" + (p.lng - track.photoLocationTolerancy).toString()
        + "&maxy=" + (p.lat + track.photoLocationTolerancy).toString()
        + "&maxx=" + (p.lng + track.photoLocationTolerancy).toString()
...
function loadTrackSync(path) {
    ...
    if(track.textToRead)
    {
        textToRead.innerHTML = $.ajax(
            {
                url:track.textToRead+"?"+Math.random(),
                async:false
            }
        ).responseText;
        get_id('textToRead', 'en', 'ml');
    }
    else
    {
        textToRead.innerHTML="";
    }
    return track;
}
...
function init(path) {
    ...
    var amOptions = {
        ...
        onStep:function (p) {
            ...
            if (counter % track.stepsToShowPhoto == 0) {
                showPhotos(track,p);
            }
        }
    };
